...
function getIcon(color: string, active: boolean): L.DivIcon {
      ...
      >
        ...
        {schoolList.map((school) => (
          <Marker
            ...
            icon={getIcon(severityColor(school.days), school.id === selectedSchoolId)}
            eventHandlers={{ click: () => onSelectSchool(school.id) }}
          >
            <Tooltip direction="top" offset={[0, -4]}>
              <span className="font-semibold">{school.name}</span> · {school.days} araw
            </Tooltip>
            <Popup>
              <div className="flex min-w-40 flex-col gap-1 text-sm">
                <span className="font-semibold text-ink">{school.name}</span>
                <span className="text-muted">{school.division} Division</span>
                <span className="text-muted">Uri: {school.type}</span>
                <span className="text-muted">{school.days} araw na nawala</span>
                <span
